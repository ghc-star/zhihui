      ...
      <div class="footer-fixed">
        ...
        <div class="all-total">
          <div class="price">
            <span>合计：</span>
            <span
              >¥ <i class="totalPrice">{{ setPrice }}</i></span
            >
          </div>
          <div
            v-if="!isEdit"
            class="goPay"
            :class="{ disabled: setCount === 0 }"
            @click="goPay"
          >
            去结算({{ setCount }})
          </div>
...
export default {
  ...
  methods: {
    ...
    deleteCart() {
      ...
      this.$store.dispatch("cart/deleteCartList");
      this.isEdit = false;
    },
    async goPay() {
      if (this.setCount > 0) {
        const cartIds = this.setCartList.map((item) => item.id).join(",");
        this.$router.push({
          path: "/pay",
          query: {
            mode: "cart",
